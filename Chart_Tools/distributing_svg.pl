...
$param{Note2Scale}=1 if (!$param{Note2Scale});
...
$g->b("svg","viewBox",
	  "0 0 ".($vbW*$param{WholeScale})." ".(($vbH+$ch*@note*$param{Note2Scale})*$param{WholeScale}),
	  "width",$vbW*$param{WholeScale},
	  "height",($vbH+($ch+$yFontSpace)*@note*$param{Note2Scale}+$yFontSpace*15)*$param{WholeScale});
...
###################
if (@note) {
	$g->b("g","transform","translate(".$XOFFSET.",".($vbH+$YOFFSET).") scale($param{Note2Scale},$param{Note2Scale})");
...
sub draw{
	...
	my @lkeys=(
		...
		"StopOffset",	# Make this rect overlap with the last one.
	);
	my %tpos=(
		LT=>1,
		LM=>1,
	...
	while (<F>) {
		...
		if (/^([^:]+):([^:]+):?(.*)$/) {
			...
			if ($draw{XZeroPos} && !exists($draw{XMove})) {
				...
				# if ($x>cut($draw{XEnd}) && !$draw{XCut}) { #??? 旧的版本，为什么要这么写呢？为什么呢？估计将来画柱状图这里会出错
				if (!$draw{XCut}) {
					$hasMore=1;
				}
				delete($xy{$x});
	...
	if ($hasMore) {
		...
		$xy{$endX}{$tmpy}{n}++;
		$xy{$endX}{$tmpy}{low}=$zeroy;
		#print "$ys[$#ys]\t$tmpy\n";
		$param{HaveMore}=$draw{HaveMore}=1;
	}
	if ($hasLess) {
		#$tmp=$startX;
		@ys=sort {$a<=>$b} keys %{$xy{$startX}};
	...
	###}
	delete($d{path}{'stroke-dasharray'});
	return $draw{StopOffset} ? "X" : $drawType;
}
